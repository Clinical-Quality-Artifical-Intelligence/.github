<img src="banner.svg" alt="Clinical Quality Artificial Intelligence — Open-Source AI for UK Nursing" width="100%" />

<div align="center">

[![Website](https://img.shields.io/badge/Website-nursingcitizendevelopment.com-005EB8?style=for-the-badge&logo=googlechrome&logoColor=white)](https://nursingcitizendevelopment.com/)
[![Hugging Face](https://img.shields.io/badge/🤗%20Hugging%20Face-NurseCitizenDeveloper-FFD21E?style=for-the-badge)](https://huggingface.co/NurseCitizenDeveloper)
[![GitHub Followers](https://img.shields.io/github/followers/Clinical-Quality-Artifical-Intelligence?style=for-the-badge&logo=github&label=Follow&color=181717)](https://github.com/Clinical-Quality-Artifical-Intelligence)
[![Location](https://img.shields.io/badge/📍_United_Kingdom-DA291C?style=for-the-badge)](#)

*Built by nurses, for nurses — free, open-source, and deployable in minutes.*

</div>

---

## 📋 Table of Contents

| | |
|---|---|
| [👋 About Us](#-about-us) | [🎯 Mission](#-our-mission) |
| [⚡ Quick Start](#-quick-start-60-seconds) | [🆕 New & Notable](#-new--notable) |
| [🛠️ Free Nursing Tools](#️-free-nursing-education-tools) | [🤖 AI & Simulation Research](#-ai--simulation-research-projects) |
| [🔬 FHIR & Standards](#-fhir--interoperability) | [🧰 Tech Stack](#-technology-stack) |
| [📋 NMC Alignment](#-nmc-standards-alignment) | [🗺️ Roadmap](#️-roadmap) |
| [👥 Who We Are](#-who-we-are) | [🙏 Acknowledgements](#-acknowledgements) |
| [🤝 Contributing](#-contributing) | [🌟 Values](#-our-values) |

---

## 👋 About Us

**Clinical Quality Artificial Intelligence** is a UK-based open-source organisation founded by **Lincoln Gombedza**, Registered Learning Disability Nurse (RNLD) and Practice Educator.

We champion the **"Nurse as Citizen Developer"** movement — empowering nurses and nursing students to build, use, and understand AI-powered clinical tools without needing a computer science degree. Every tool we build is free, requires no API keys (unless stated), and is deployable to Hugging Face Spaces in minutes.

> 💡 *"Technology built by those who understand the ward is technology that works on the ward."*

---

## 🎯 Our Mission

> To make high-quality, evidence-based digital tools freely available to every nursing student and registered nurse in the UK — and beyond.

| | |
|-|-|
| 📚 | **Enhance nursing education** with interactive, evidence-based learning tools |
| 🏥 | **Support clinical practice** with decision-support and reference tools |
| 🤖 | **Advance AI literacy** in nursing through hands-on, accessible technology |
| 🌈 | **Promote health equity** by removing cost and access barriers |
| 🔓 | **Stay open source** — transparent, collaborative, community-driven |

---

## ⚡ Quick Start — 60 Seconds

No login. No API key. No setup. Pick any tool and launch it directly on Hugging Face Spaces:

```
1. Click any 🤗 badge below
2. The Streamlit app loads in your browser
3. Start learning / practising immediately
```

> All tools are hosted on [Hugging Face Spaces](https://huggingface.co/NurseCitizenDeveloper) and run entirely in the cloud. A modern browser is all you need.

**Want to run a tool locally or contribute?**
```bash
git clone https://github.com/Clinical-Quality-Artifical-Intelligence/<repo-name>
pip install -r requirements.txt
streamlit run app.py
```

---

## 🆕 New & Notable

<table>
<tr>
<td width="50%">

### 🏆 NurseSim-RL — OpenEnv Challenge 2026
AI-powered clinical triage simulation using the Manchester Triage System. Gymnasium-compatible reinforcement learning environment with A2A protocol support — our competition entry for the **OpenEnv Challenge 2026**.

[![Repo](https://img.shields.io/badge/View%20Repo-NurseSim--RL-181717?style=flat-square&logo=github)](https://github.com/Clinical-Quality-Artifical-Intelligence/NurseSim-RL)
[![Leaderboard](https://img.shields.io/badge/Live%20Leaderboard-🏆-FFD700?style=flat-square)](https://github.com/Clinical-Quality-Artifical-Intelligence/NurseSim-Triage-Leaderboard)

</td>
<td width="50%">

### 📐 Open Nursing FHIR IG
Foundational set of FHIR R4 profiles implementing the **ADPIE nursing process** (Assessment, Diagnosis, Planning, Intervention, Evaluation) as interoperable healthcare data. Bridging HL7 FHIR and nursing science.

[![Repo](https://img.shields.io/badge/View%20Repo-open--nursing--core--ig-181717?style=flat-square&logo=github)](https://github.com/Clinical-Quality-Artifical-Intelligence/open-nursing-core-ig)
[![FHIR](https://img.shields.io/badge/FHIR-R4-E8523A?style=flat-square)](https://hl7.org/fhir/R4/)

</td>
</tr>
<tr>
<td width="50%">

### 🖥️ NurseStack CLI
A developer toolkit and skills platform for Nurse Citizen Developers. Deploy, manage, and extend CQAI tools from your terminal.

[![Repo](https://img.shields.io/badge/View%20Repo-nursestack-181717?style=flat-square&logo=github)](https://github.com/Clinical-Quality-Artifical-Intelligence/nursestack)
[![Shell](https://img.shields.io/badge/Shell-Bash-4EAA25?style=flat-square&logo=gnubash&logoColor=white)](#)

</td>
<td width="50%">

### 🧬 Genomics Equity Paper
*"Closing the Genomic Gap"* — a practice development paper on pharmacogenomics equity in diverse NHS populations. Open-access, peer-written by the CQAI team.

[![Read Paper](https://img.shields.io/badge/Read%20Paper-cqai--genomics-181717?style=flat-square&logo=github)](https://github.com/Clinical-Quality-Artifical-Intelligence/cqai-genomics-paper)

</td>
</tr>
</table>

---

## 🛠️ Free Nursing Education Tools

*A growing suite of free Streamlit apps — open source, no API keys, no login required.*

<details open>
<summary><strong>📖 Research & Academic Writing</strong></summary>
<br>

| Tool | Description | Launch |
|------|-------------|--------|
| **[EBP Research Tool](https://github.com/Clinical-Quality-Artifical-Intelligence/nursing-ebp-tool)** | Multi-database search (PubMed + ClinicalTrials.gov), evidence grading (Melnyk I–VII), Vancouver / Harvard / APA 7th citations, PRISMA 2020 flow tracker | [![HF](https://img.shields.io/badge/🤗-Open%20App-FFD21E?style=flat-square)](https://huggingface.co/spaces/NurseCitizenDeveloper/nursing-ebp-tool) |
| **[AI Research & Academic Writer](https://github.com/Clinical-Quality-Artifical-Intelligence/nursing-research-writer)** | Literature review, essay writer, dissertation, reflective accounts (Gibbs/Driscoll/Johns), NMC Revalidation CPD, NMC 7-platform mapping, Vancouver + Harvard citations. BYOK (Claude API) | [![HF](https://img.shields.io/badge/🤗-Open%20App-FFD21E?style=flat-square)](https://huggingface.co/spaces/NurseCitizenDeveloper/nursing-research-writer) |

</details>

<details open>
<summary><strong>💊 Medicines Management</strong></summary>
<br>

| Tool | Description | Launch |
|------|-------------|--------|
| **[Drug Card Generator](https://github.com/Clinical-Quality-Artifical-Intelligence/nursing-drug-cards)** | FDA + RxNorm drug cards with nursing considerations for 12+ classes, black box warnings, drug interactions, side-by-side compare | [![HF](https://img.shields.io/badge/🤗-Open%20App-FFD21E?style=flat-square)](https://huggingface.co/spaces/NurseCitizenDeveloper/nursing-drug-cards) |
| **[Drug Calculations Trainer](https://github.com/Clinical-Quality-Artifical-Intelligence/nursing-drug-calc)** | 7 UK calculation types: tablets, IV mL/hr, drip drops/min, mg/kg, mcg/kg/min, % solutions. Infinite dynamic questions, exam mode (5–30 Qs), step-by-step worked solutions | [![HF](https://img.shields.io/badge/🤗-Open%20App-FFD21E?style=flat-square)](https://huggingface.co/spaces/NurseCitizenDeveloper/nursing-drug-calc) |
| **[Medication Safety Checker](https://github.com/Clinical-Quality-Artifical-Intelligence/medication-safety)** | ISMP 2023 high-alert meds, RxNorm drug interaction API, dose safety checker (30+ drugs), LASA warnings (30+ pairs), Nine Rights checklist | [![HF](https://img.shields.io/badge/🤗-Open%20App-FFD21E?style=flat-square)](https://huggingface.co/spaces/NurseCitizenDeveloper/medication-safety) |

</details>

<details open>
<summary><strong>🏥 Clinical Skills & Assessment</strong></summary>
<br>

| Tool | Description | Launch |
|------|-------------|--------|
| **[NEWS2 Clinical Deterioration Hub](https://github.com/Clinical-Quality-Artifical-Intelligence/nursing-news2)** | Full RCP 2017 NEWS2 calculator (SpO₂ Scale 1 & 2), ABCDE primary survey + Sepsis Six, SBAR handover builder with Word .docx download, clinical reference guide | [![HF](https://img.shields.io/badge/🤗-Open%20App-FFD21E?style=flat-square)](https://huggingface.co/spaces/NurseCitizenDeveloper/nursing-news2) |
| **[Clinical Assessment Scales Hub](https://github.com/Clinical-Quality-Artifical-Intelligence/nursing-assessment-scales)** | 7 validated scales: Waterlow, MUST, Morse, STRATIFY, Abbey Pain Scale, PHQ-9 (suicidal ideation safety alert), MMSE — all with colour-coded risk levels and management guidance | [![HF](https://img.shields.io/badge/🤗-Open%20App-FFD21E?style=flat-square)](https://huggingface.co/spaces/NurseCitizenDeveloper/nursing-assessment-scales) |
| **[Case Study Builder](https://github.com/Clinical-Quality-Artifical-Intelligence/nursing-case-studies)** | 10 clinical cases (STEMI, DKA, Sepsis, Stroke, AKI, NOF fracture…), ADPIE reasoning with model answers, NCLEX-style MCQs, NANDA care plan builder | [![HF](https://img.shields.io/badge/🤗-Open%20App-FFD21E?style=flat-square)](https://huggingface.co/spaces/NurseCitizenDeveloper/nursing-case-studies) |

</details>

<details open>
<summary><strong>🎓 Exam Preparation & Professional Registration</strong></summary>
<br>

| Tool | Description | Launch |
|------|-------------|--------|
| **[NCLEX Practice Question Generator](https://github.com/Clinical-Quality-Artifical-Intelligence/nclex-prep)** | MCQ, SATA, dosage calculations, category & difficulty filters, full rationales, score breakdown, wrong answer review | [![HF](https://img.shields.io/badge/🤗-Open%20App-FFD21E?style=flat-square)](https://huggingface.co/spaces/NurseCitizenDeveloper/nclex-prep) |
| **[NMC CBT Practice Questions](https://github.com/Clinical-Quality-Artifical-Intelligence/nursing-nmc-cbt)** | 30 expert-written MCQs across all 7 NMC platforms + NMC Code, full rationales with NMC/NICE/UK law references, platform & difficulty filters, exam mode | [![HF](https://img.shields.io/badge/🤗-Open%20App-FFD21E?style=flat-square)](https://huggingface.co/spaces/NurseCitizenDeveloper/nursing-nmc-cbt) |

</details>

---

## 🤖 AI & Simulation Research Projects

| Project | Description | Status |
|---------|-------------|--------|
| **[NurseSim-RL](https://github.com/Clinical-Quality-Artifical-Intelligence/NurseSim-RL)** | AI clinical triage simulation using Manchester Triage System. OpenEnv Challenge 2026 entry with A2A protocol, Gymnasium-compatible RL environment | 🏆 Competition Entry |
| **[Nursing Council Agent](https://github.com/Clinical-Quality-Artifical-Intelligence/nursing-council-agent)** | Multi-perspective AI peer review tool simulating academic, clinical, and student voices for nursing education quality assurance | ⚡ Active |
| **[Professional Nurse Advocate Assistant](https://github.com/Clinical-Quality-Artifical-Intelligence/Professional-Nurse-Advocate-Assistant)** | AI assistant supporting Professional Nurse Advocates (PNA) in restorative clinical supervision | 🔬 Development |
| **[Health Inclusion Coach](https://github.com/Clinical-Quality-Artifical-Intelligence/Health-Inclusion-Coach)** | AI-powered platform helping NHS teams embed inclusion, equity, and belonging into clinical practice and leadership | 💡 Active |
| **[Open Nursing Validator](https://github.com/Clinical-Quality-Artifical-Intelligence/open-nursing-validator)** | TypeScript/Node.js FHIR-compliant validation service for nursing-specific healthcare data | 🔧 In Development |
| **[Nursing Language Translator](https://github.com/Clinical-Quality-Artifical-Intelligence/nursing-language-translator)** | Clinical shorthand and nursing terminology translation powered by NurseEmbed-300M | 🔧 In Development |

---

## 🔬 FHIR & Interoperability

<details>
<summary><strong>Open Nursing Core Implementation Guide — FHIR R4</strong></summary>
<br>

The **[Open Nursing Core IG](https://github.com/Clinical-Quality-Artifical-Intelligence/open-nursing-core-ig)** is a foundational FHIR Implementation Guide that formalises the nursing process (ADPIE) as interoperable, standards-compliant healthcare data.

| Profile | Description |
|---------|-------------|
| `NursingAssessment` | Structured clinical assessment aligned to ADPIE |
| `NursingDiagnosis` | NANDA-I–aligned nursing diagnosis resource |
| `NursingCarePlan` | Goal-directed care planning with SMART objectives |
| `NursingIntervention` | Evidence-linked nursing interventions (NIC-aligned) |
| `NursingOutcome` | Measurable care outcomes (NOC-aligned) |

Built with **FHIR Shorthand (FSH)**, published via **SUSHI**, and validated against the HL7 FHIR R4 specification. Aligned with NHS Digital FHIR UK Core.

[![FHIR](https://img.shields.io/badge/FHIR-R4-E8523A?style=flat-square)](https://hl7.org/fhir/R4/)
[![NHS UK Core](https://img.shields.io/badge/NHS%20UK%20Core-Aligned-005EB8?style=flat-square)](https://simplifier.net/hl7fhirukcorer4)
[![Licence](https://img.shields.io/badge/Licence-MIT-green?style=flat-square)](https://opensource.org/licenses/MIT)

</details>

---

## 🧰 Technology Stack

<div align="center">

| Layer | Technologies |
|-------|-------------|
| **Languages** | Python · TypeScript · JavaScript · FSH (FHIR Shorthand) · Shell |
| **AI / ML** | PyTorch · Transformers · Anthropic Claude API · LiteLLM · Gymnasium (RL) |
| **Healthcare Standards** | FHIR R4 · HL7 · NMC Standards (2018) · RCP NEWS2 · NICE Guidelines · BAPEN MUST |
| **Clinical APIs** | NCBI PubMed · ClinicalTrials.gov · openFDA · RxNorm/NLM · NIH ISMP |
| **Web & Apps** | Streamlit · Gradio · React · Vue.js · Docusaurus · python-docx |
| **CI/CD & Cloud** | GitHub Actions · Hugging Face Spaces · Docker · Azure · Cloudflare |

</div>

<div align="center">

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![FHIR](https://img.shields.io/badge/FHIR-E8523A?style=flat-square)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat-square&logo=streamlit&logoColor=white)
![Hugging Face](https://img.shields.io/badge/🤗%20Hugging%20Face-FFD21E?style=flat-square)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)

</div>

---

## 📋 NMC Standards Alignment

<details>
<summary><strong>View full NMC Platform mapping</strong></summary>
<br>

All student-facing tools are mapped to the **NMC Standards of Proficiency for Registered Nurses (2018)**:

| NMC Platform | Tools |
|---|---|
| **Platform 1** — Being an accountable professional | NMC CBT Practice · AI Research Writer |
| **Platform 2** — Promoting health and preventing ill health | NMC CBT Practice · Assessment Scales (PHQ-9) |
| **Platform 3** — Assessing needs and planning care | Assessment Scales · Case Studies · NEWS2 Hub · EBP Research Tool |
| **Platform 4** — Providing and evaluating care | Drug Calc Trainer · Drug Cards · NEWS2 Hub · Medication Safety |
| **Platform 5** — Leading and managing nursing care | NMC CBT Practice · AI Research Writer |
| **Platform 6** — Improving safety and quality of care | Medication Safety · NEWS2 Hub · Assessment Scales |
| **Platform 7** — Coordinating care | NMC CBT Practice · SBAR Builder (NEWS2 Hub) |

</details>

---

## 🗺️ Roadmap

<details>
<summary><strong>What's coming next</strong></summary>
<br>

### 2025 Q3–Q4 — In Progress
- [ ] **NurseSim-RL v2** — expanded triage scenarios, multi-agent environment
- [ ] **NHS Preceptorship Support Platform** — structured reflection and competency tracking for newly qualified nurses
- [ ] **Open Nursing FHIR IG v1.0** — first stable release with full ADPIE profiles
- [ ] **NurseStack v1.0** — stable CLI release with deploy, test, and scaffold commands

### 2026 Q1–Q2 — Planned
- [ ] **Nursing Language Translator** — NurseEmbed-300M production release
- [ ] **CQAI Community Hub** — forum and peer support space for Nurse Citizen Developers
- [ ] **NMC Revalidation Assistant** — AI-supported portfolio builder and CPD tracker
- [ ] **Clinical Placement Learning Tracker** — PARE-aligned digital logbook

### Always open
- 💬 [Suggest a new tool or feature](https://github.com/Clinical-Quality-Artifical-Intelligence/.github/issues/new?template=feature_request.md)
- 🐛 [Report a clinical inaccuracy](https://github.com/Clinical-Quality-Artifical-Intelligence/.github/issues/new?template=clinical_inaccuracy.md)
- 🤝 [Contribute to an existing project](CONTRIBUTING.md)

</details>

---

## 👥 Who We Are

### Founder & Lead Developer

**Lincoln Gombedza, RNLD** — Registered Learning Disability Nurse | Practice Educator | Nurse Citizen Developer

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Lincoln%20Gombedza-0077B5?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/lincoln-gombedza)
[![GitHub](https://img.shields.io/badge/GitHub-@ClinyQAi-181717?style=flat-square&logo=github)](https://github.com/ClinyQAi)
[![Email](https://img.shields.io/badge/Email-Contact-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:nursingcitizendevelopment@gmail.com)

---

### Key Collaborators

<table>
<tr>
<td align="center" width="25%">

**Kelly Thobekile Ncube, RN**
Senior Lecturer in Adult Nursing (SFHEA)
Global Health Lecturer Volunteer Fellow

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/kelly-thobekile-ncube-24502069/)

</td>
<td align="center" width="25%">

**Kumbi Kariwo, FRCN QN**
Health Inequalities & AI Equity Lead

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/kumbi-kariwo-frcn-qn-46bb72167/)

</td>
<td align="center" width="25%">

**Hector Musonza**
Practice Education & Preceptorship Lead

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/fungai-hector-musonza-4a573371/)

</td>
<td align="center" width="25%">

**Dr Hellena Habte-Asres**
Lead Clinical Academic Research Nurse
Diabetes & CKD Specialist

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/dr-hellena-habte-asres-3698491a3/)

</td>
</tr>
<tr>
<td align="center" width="25%">

**Winnie George**
National Programme Lead (Retention)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/winnie-george-60213993/)

</td>
<td align="center" width="25%">

*Your name here?*
We welcome clinical collaborators, educators, and developers.

[![Contribute](https://img.shields.io/badge/Get%20Involved-005EB8?style=flat-square)](CONTRIBUTING.md)

</td>
<td></td>
<td></td>
</tr>
</table>

---

## 🙏 Acknowledgements

We are grateful to our mentors and champions across the nursing and healthcare innovation community:

**Aisha Holloway** — Chief Nursing Officer for Scotland &nbsp;·&nbsp;
**Prof. Joanne Bosanquet** — CEO, Foundation of Nursing Studies &nbsp;·&nbsp;
**Dr Clare Cable** — CEO, Burdett Trust for Nursing &nbsp;·&nbsp;
**Prof. Gemma Stacey** — Programme Director, Nursing Now Challenge &nbsp;·&nbsp;
**Josie Rudman MBE** — NHS England, Mutual Mentor &nbsp;·&nbsp;
**Rohit Sagoo** — Children's Nurse & Innovation Mentor

*Supported by: Foundation of Nursing Studies (FoNS) · Burdett Trust for Nursing · Royal College of Nursing (RCN) · Nursing Now Challenge · Global Health Mentorships*

---

## 🤝 Contributing

We welcome contributions from nurses, nursing educators, healthcare technologists, and developers at all levels. **No computer science degree required.**

| How to contribute | |
|---|---|
| ⭐ **Star a repository** | Show support and help others discover these tools |
| 🐛 **[Open an issue](https://github.com/Clinical-Quality-Artifical-Intelligence/.github/issues/new/choose)** | Report a bug, clinical inaccuracy, or suggest a feature |
| 🔀 **Submit a pull request** | Add questions, fix bugs, improve UI — all welcome |
| 📖 **Improve documentation** | Clinical accuracy in descriptions matters enormously |
| 💬 **[Start a discussion](https://github.com/orgs/Clinical-Quality-Artifical-Intelligence/discussions)** | Help shape the roadmap and ethical AI priorities |
| 🌐 **Translate content** | Help reach non-English-speaking nursing communities |

Please read our **[Contributing Guide](https://github.com/Clinical-Quality-Artifical-Intelligence/.github/blob/main/CONTRIBUTING.md)** and **[Code of Conduct](https://github.com/Clinical-Quality-Artifical-Intelligence/.github/blob/main/CODE_OF_CONDUCT.md)** before contributing.

All projects use the **MIT Licence** unless stated.

---

## 🌟 Our Values

| | |
|-|-|
| 💙 | **Person-Centred** — patients and families at the heart of everything we build |
| 📊 | **Evidence-Based** — grounded in validated clinical tools and UK guidelines |
| 🌈 | **Equitable** — removing financial and access barriers to quality education |
| 🔓 | **Open Source** — transparent, auditable, community-owned |
| ⚖️ | **Ethical AI** — responsible, explainable, and clinically appropriate |
| 🎓 | **Lifelong Learning** — supporting every stage of a nursing career |
| 🤝 | **Community-First** — built with and for the nursing community, not just for it |

---

<div align="center">

**Made with 💙 for the UK nursing community and nursing students worldwide**

*If these tools help even one student pass their medicines management exam, it's worth it.*

[![Explore All Tools on Hugging Face](https://img.shields.io/badge/🤗%20Explore%20All%20Tools-Hugging%20Face-FFD21E?style=for-the-badge)](https://huggingface.co/NurseCitizenDeveloper)
[![Website](https://img.shields.io/badge/Website-nursingcitizendevelopment.com-005EB8?style=for-the-badge&logo=googlechrome&logoColor=white)](https://nursingcitizendevelopment.com/)
[![Contribute](https://img.shields.io/badge/Contribute-Open%20Source-2ea44f?style=for-the-badge&logo=github)](https://github.com/Clinical-Quality-Artifical-Intelligence/.github/blob/main/CONTRIBUTING.md)

</div>

<img src="banner.svg" alt="" width="100%" />
